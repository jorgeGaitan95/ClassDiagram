...
define([
	...
	function resizeImage() {
		...
		lib.empty(imageTile);
		imageTile.appendChild(document.createTextNode(imgW + " x " + imgH + " pixelesJOrge -- " + Math.floor(img.width/imgW*100) + "%"));
	}*/
	//window.onresize = resizeImage;
	var commandsProxy = new mCommandsProxy.CommandsProxy();
	
	var EDITOR_ID = "orion.viewer.model"; //$NON-NLS-0$
	
	var headers = {
		name: "Orion Model Viewer Plugin",
		version: "1.0", //$NON-NLS-0$
		description: "This plug-in provides a viewer for models files."
	};
	...
	provider.registerService("orion.edit.editor", { //$NON-NLS-0$
		...
		name: messages["Orion model Viewer"], //$NON-NLS-0$
	...
	provider.registerService("orion.navigate.openWith", {}, { //$NON-NLS-0$
		editor: EDITOR_ID,
		contentType: ["application/json","text/html"] //$NON-NLS-5$ //$NON-NLS-4$ //$NON-NLS-3$ //$NON-NLS-2$ //$NON-NLS-1$ //$NON-NLS-0$
	});
